...
vehicle.commands.add(Command( 0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, 0, DEFAULT_ALTITUDE))
add_goto_mission(vehicle)
#copies last mission, for mission control purposes
vehicle.commands.add(vehicle.commands[vehicle.commands.count - 1])
vehicle.commands.upload()
...
while True:
	currentMission = vehicle.commands.next -1
	missionitem = vehicle.commands[currentMission] #commands are zero indexed
	if currentMission == (vehicle.commands.count - 1):
		break
...
while True:
	...
	distance = distance + (height,)
	print "distances: lon = %f, lat = %f, alt = %f" % distance
	if fabs(vehicle.location.global_relative_frame.alt) <= Constants.MAX_LANDING_ALTITUDE:
		break
	time.sleep(1)
